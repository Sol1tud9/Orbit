...
Работающее приложение: [Orbita на VPS](http://72.56.3.59). Публикация по IP использует HTTP. Для HTTPS с доменом предусмотрен отдельный compose.public.yaml.
...
Текущая публикация по IP доступна на http://72.56.3.59. В материалах организаторов требуется доступность сервиса до завершения проверки жюри. HTTPS с доменом пока не настроен.

### Текущий VPS: запуск по IP

Сервер Ubuntu, 4 CPU и 8 GB RAM. Исходники размещены в `/opt/orbita/releases/20260913-db28217`, ссылка `/opt/orbita/current` указывает на текущий выпуск. Рабочие данные хранятся в Docker-томе `orbita_orbita-data`, независимо от каталога выпуска.

Для управления подключиться по SSH, затем:

```sh
cd /opt/orbita/current
docker compose -p orbita -f compose.yaml -f compose.ip.yaml ps
docker compose -p orbita -f compose.yaml -f compose.ip.yaml logs --tail 100 app
```

В серверном .env задано `ORBITA_HOST=72.56.3.59`. compose.ip.yaml публикует Caddy на порту 80, backend доступен только локально на 8000. Контейнер расчёта ограничен 3 CPU и 4 GB RAM; журналы ротируются. Docker включён в автозапуск, контейнеры используют restart: unless-stopped.

После обновления файлов текущего выпуска:

```sh
docker compose -p orbita -f compose.yaml -f compose.ip.yaml up -d --build
```

Для нового выпуска предпочтительно создать отдельный каталог в releases и сохранить имя Compose-проекта `orbita`. Не использовать `down -v`: это удалит результаты. Для перехода на домен использовать compose.public.yaml вместо compose.ip.yaml и указать ORBITA_DOMAIN. DNS домена должен указывать на сервер.
...
- Публикация по IP выполнена на VPS. Длительная нагрузочная проверка и HTTPS с доменом остаются отдельными задачами эксплуатации.
