...
`optimization.py` добавляет воспроизводимую выборку из конечной сетки параметров, обычный полный пересчёт кандидатов и точное сравнение четырёх критериев Парето. В API добавлен вид эксперимента `optimization` со строгим объектом `options`. Миграция SQLite v3 добавляет сериализованные настройки поиска; база, кандидаты и доказательства хранятся в общем формате артефактов экспериментов. Ограничения очередей, владения, работы и отмены остаются общими. Подробная политика: [AUTOPICK.md](AUTOPICK.md).

`OptimizationView.tsx` отделяет UX поиска от устойчивости и обычных расчётов. `themes.css` задаёт мягкие светлую и тёмную палитры и контраст контактов/подписей. Тема сохраняется в localStorage и применяется до запуска React; она не входит в математический сценарий.
